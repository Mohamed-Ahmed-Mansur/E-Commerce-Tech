just test branch

back-end requirements:

1- create a good UI for the admin panel
2- open a live connection with the database and fill it with data (MongoDB)
3- create CRUD operations for customers, products, and orders (NestJs, NodeJs, or Express)
4- Authorization and Authentication for login page using JWT (Token)

Mariam ==> Create JSON file of products (Name, Description, Price, Image: "src"), Add JSON file (our data) to MongoDB

Test merge is working + resolve conflicts

